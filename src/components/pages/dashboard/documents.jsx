            ...
            props.docs
              ...
              .map((doc, index) => (
                <Document
                  ...
                  openedDocsCommit={{
                    value: openedDocsCommit,
                    set: setOpenedDocsCommit,
                  }}
                />
              ))
          ) : (
                ...
                >
                  ...
                  <CustomTableBody>
                    <TableRow>
                      <TableCell>
                        <Tooltip title={preview.commiter?.email || ""} arrow>
                          <Chip
                            avatar={
                              <Avatar
                                alt={preview.commiter?.name}
                                src={preview.commiter?.avatar}
                      ...
                      <TableCell>
                        {processTimeForDashboard(preview.commit?.created_at)}
                      </TableCell>
                      <TableCell>
                        {processTimeForDashboard(preview.commit?.last_modified)}
                      </TableCell>
                    </TableRow>
                  </CustomTableBody>
                </Table>
  ...
  return (
    <>
      <TableRow sx={{ "& > *": { borderBottom: "unset" } }}>
        ...
        <TableCell align="left" sx={{ whiteSpace: "nowrap" }}>
          {processTimeForDashboard(doc.created_at)}
        </TableCell>
        <TableCell align="left" sx={{ whiteSpace: "nowrap" }}>
          {processTimeForDashboard(doc.last_modified)}
        </TableCell>
        <TableCell align="left" sx={{ whiteSpace: "nowrap" }}>
          {doc.isPublic ? (
            <Chip
        ...
        <TableCell align="left" sx={{ whiteSpace: "nowrap" }}>
          <Chip
            label={`${doc.commits?.length} Commit Found`}
            variant="outlined"
            color="success"
            size="small"
          />
      ...
      <TableRow
        sx={(theme) => ({
          backgroundColor:
            theme.palette.mode === "dark" ? "#2c2f35" : "#e2e2e2",
        })}
      >
        <TableCell style={{ paddingBottom: 0, paddingTop: 0 }} colSpan={8}>
          <Collapse
            in={openedDocsCommit.value === index}
            timeout="auto"
            unmountOnExit
          >
            <Box sx={{ margin: 1 }}>
                        ...
                        function Child(props) {
                          ...
                          React.useEffect(() => {
                            if (auth.user.email === commit.commiter) {
                              ...
                              commit.commiter === props.state.owner?.email
                            ) {
                              setState((state) => ({
                                ...state,
                          ...
                          return (
                            <TableRow>
                              <TableCell>
                                <Tooltip
                                  title={state.commiter.email || ""}
                                  arrow
                                >
                                  <Chip
                                    avatar={
                                      <Avatar
                                        alt={state.commiter.name}
                                        src={state.commiter.avatar}
                                        imgProps={{
                              ...
                              <TableCell>
                                <Link
                                  to={`/doc/${doc.id}?commit=${commit.id}`}
                                  style={{
                                    color: theme.palette.text.primary,
                                  }}
                                >
                              ...
                              <TableCell>
                                {processTimeForDashboard(commit.created_at)}
                              </TableCell>
                              <TableCell>
                                {processTimeForDashboard(commit.last_modified)}
                              </TableCell>
